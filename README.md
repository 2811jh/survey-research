...
> ⚠️ **注意**：1.需要 [Node.js](https://nodejs.org/) >= 18 ；2.裸命令会进入交互式多选 UI，而该 UI 需要 **按空格键** 勾选 agent（仅按回车不会勾选！）。
...
```bash
# Claude Code 用户（装到 ~/.claude/skills/）
npx skills add 2811jh/survey-research -g --agent claude-code -y

# CodeMaker 用户（装到 ~/.codemaker/skills/）
npx skills add 2811jh/survey-research -g --agent codemaker -y

# 一次装到多个 agent
npx skills add 2811jh/survey-research -g --agent claude-code,codemaker,cursor -y
...
> 
> 需要时间序列对比时还能：📅 异动诊断（按周/月/天/季度/自定义区间/列分桶对比各题异动）

**你只说一句话，工具跑完核心五个阶段（+异动诊断按需）。中间不打断、不追问、不需要你操心任何参数。**
...
**不知道分组列名？** 传 `--col_questions '["auto"]'`，脚本自动识别人口学题（性别/年龄/职业/付费/会员/渠道等），返回候选清单让你确认，再重跑。
...
| 📁 **交叉分析 Excel** | 5 Sheet：交叉分析/列百分比（含显著性着色）/得分分析/差异热力图/显著性检验 |
...
### 📅 6. 异动诊断 — 时间序列对比与版本/渠道分桶

除了常规分析，工具还支持**问卷异动诊断**，按时间或任意离散列分桶，逐题做相邻期显著性检验：

- **时间分桶**：按周/月/天/季度/自定义区间（如双 11 前后）对比各期异动
- **列分桶**：按版本号/活动批次/渠道/用户分层等离散列对比各桶异动
- **双门槛判定**：p<0.05 且（占比 Δ≥5pp 或 均分 Δ≥0.1）才判异动，避免微波动误报
- **自动识别**：五点量表题自动纳入均分检验；时间列自动检测（默认列 + 关键词扫描 + ask 兜底）
- **4-Sheet Excel**：指标总览 / 逐题异动明细（整体基线 + 逐期热力 + 样本量行）/ 异动汇总 / 方法与样本

典型场景：回流问卷月度跟踪、满意度季度对比、NPS 月度对比、活动前后对比、版本 A vs 版本 B、渠道对比。
...
│   ├── survey_drift.py        # 异动诊断：按周/月/天/季度/自定义区间/列分桶，逐题显著性检验 + 导出 4-Sheet Excel
...
└── references/           # 方法论参考文档
    ...
    └── 18-drift-workflow.md       # 问卷异动诊断流程：时间分桶（周/月/天/季度/自定义区间）+ 列分桶对比
```
...
| `问卷异动诊断_{按周/月/天/季度/自定义区间/分桶列}_{时间戳}.xlsx` | 异动诊断 4-Sheet：指标总览 / 逐题异动明细（整体基线列 + 逐期热力 + 样本量行）/ 异动汇总 / 方法与样本 |
