...
description: |
  ...
  当用户说"按周/月/天/季度诊断这份问卷数据的变化"、"逐题对比各周/各月的满意度和 NPS 有没有显著变化"、
  "问卷数据有没有异常波动"、"哪个指标这期掉了/涨了、显著吗"、"异动分析"、"回流报告"、
...
4. **自动进入分析流程**：
   ...
   **不需要用户再手动指定路径**——下载完直接开始分析，一气呵成。
   如用户明确要求「按周/月对比」「异动诊断」「版本对比」等，可直接跳到阶段 6，
...
**触发条件**：用户明确要求对比不同人群（如"分析不同性别/付费玩家的差异"）、"请你重点分析一下不同 xxx"、
或用户要求全面分析且数据中存在明显的分组变量。支持 `--col_questions '["auto"]'` 自动识别人口学题（性别/年龄/职业/付费/会员/渠道等）。
...
**触发条件**：用户有含时间列的问卷数据（回流/满意度/NPS/活动跟踪等），或想按任意离散列对比各桶异动。
典型话术："按周/月/天/季度/自定义区间诊断这份问卷数据的变化"、"逐题对比各月满意度和 NPS 有没有显著变化"、
"问卷数据有没有异常波动"、"对比版本 A vs 版本 B 的满意度异动"、"活动前后对比"、"渠道对比"。
...
• 做异动诊断（按周/月/天/季度对比满意度/NPS/单选/多选，或按版本/渠道列分桶对比，定位显著变化并写初步结论）
...
| `references/18-drift-workflow.md` | 问卷异动诊断流程：时间分桶（周/月/天/季度/自定义区间）+ 列分桶对比（阶段 6） |
