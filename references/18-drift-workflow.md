...
| 📈 逐题异动明细 | 每题×各期占比（DataBar）+ **C 列「整体」基线**（全样本各期按样本量加权的整体占比，供各期对照）+ **单选/多选选项按整体占比降序排**（五点量表题、人口题「性别/年龄/职业」保持原顺序）+ **逐周环比热力标注**（某周相对前一周显著变化则着色：琥珀底加粗=大幅异动、红/绿字=一般显著升降、灰字=无变化）+ 异动周列 + **每题末行「样本量」**（整体 N + 各期有效 n）+ **五点量表题再加「加权满意度」行**（Σ分值×人数/总样本量 = 1~5 均分，整体 + 各期）+ **AI 结论列** |
