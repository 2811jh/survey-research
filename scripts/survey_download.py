...
def _safe_name(name, max_len=20):
    """将问卷名称转换为安全的文件名片段（去书名号、非法字符，截断）"""
    if not name:
        return ""
    # 去除书名号和中文括号
    name = re.sub(r'[《》「」『』【】〔〕]', '', name)
...
def _is_watermark_line(text):
    """检测一行是否为问卷系统下载文件的页脚水印。

    服务端导出的 CSV/XLSX 末尾会附加一行水印：
      网易内部文件，泄密必究！！！(NetEase Internal documents must be investigated for leakage!!!)
    及其前后的若干空行。合并分片时这些行会污染数据，需要过滤。

    采用「整条水印拼接」匹配，避免误伤含 'leakage' / 'internal' 等英文单词
    的真实用户答案。两段水印来自原文不同语言部分，命中任一即视为水印行。

    text: 字符串（CSV 行或 XLSX 行的拼接文本）
    """
    if not text:
        return False
    s = str(text)
    # 整条水印拼接（中英两段，命中任一即视为水印）
    watermark_patterns = (
        "网易内部文件，泄密必究",                         # 中文段
        "NetEase Internal documents must be investigated",  # 英文段
    )
    return any(p in s for p in watermark_patterns)
...
def _merge_csv_files(file_list, output_path):
    ...
    合并多个 CSV 分片文件为一个文件。
    第 2 个及以后的文件跳过表头行。
    自动过滤每个分片末尾的水印行（"网易内部文件，泄密必究！！！"）和连续空行。
    """
    file_list.sort()
    ...
    with open(output_path, 'w', encoding='utf-8-sig', newline='') as out:
        for i, fpath in enumerate(file_list):
            with open(fpath, 'r', encoding=encoding) as inp:
                # 整个分片读完后再写出，便于剥离末尾的水印 + 空行
                lines = inp.readlines()
            # 从末尾向前剥离水印行 + 空行 + 仅含分隔符的空 CSV 行
            while lines:
                last = lines[-1].rstrip('\r\n')
                stripped = last.replace(',', '').replace('"', '').strip()
                if _is_watermark_line(last):
                    lines.pop()
                    skipped_watermark += 1
                    continue
                if stripped == '':
            ...
            for j, line in enumerate(lines):
                if i > 0 and j == 0:
                    continue  # 后续文件跳过表头
                # 即便不在末尾，行内若命中水印关键字也丢弃
                if _is_watermark_line(line):
                    skipped_watermark += 1
                    continue
                out.write(line)
                total_lines += 1
    if skipped_watermark or skipped_blank:
        _log(f"  Cleaned: {skipped_watermark} watermark line(s), {skipped_blank} trailing blank line(s)")
    # 删除原始分片
    for f in file_list:
        os.remove(f)
...
def _clean_shard_dataframe(df):
    """清理单个 XLSX 分片的 DataFrame：移除末尾水印行 + 末尾全空行。
    ...
    try:
        ...
        return df
    n_before = len(df)
    # 把每一行所有单元格拼成一行字符串，用于水印检测
    row_text = df.astype(str).fillna('').agg(' '.join, axis=1)
    # 标记水印行
    is_watermark = row_text.apply(_is_watermark_line)
    df = df[~is_watermark].reset_index(drop=True)
    # 移除末尾连续的全空行
    while len(df) > 0 and df.iloc[-1].isna().all():
        df = df.iloc[:-1]
    n_after = len(df)
    if n_before != n_after:
        _log(f"    Cleaned shard: dropped {n_before - n_after} watermark/blank row(s)")
    return df
...
def _merge_xlsx_files(file_list, output_path):
    ...
    每个分片读取后会自动过滤末尾水印行（网易内部文件，泄密必究!!!）和全空行。
    """
    try:
        import pandas as pd
    except ImportError:
        ...
        if total_size_mb > 15:
            ...
            with open(csv_output, 'w', encoding='utf-8-sig', newline='') as out_f:
                ...
                for fpath in file_list:
                    ...
                    chunks = _clean_shard_dataframe(chunks)
...
# 年龄选项分类关键词
_AGE_YOUNG_KEYWORDS = ['14岁', '15', '16', '17', '岁以下']  # 18岁以下
...
_JOB_WORKING_KEYWORDS = [
    ...
    # 注：「自由职业/自雇/自媒体」和「无固定工作/兼职」不计入工作人群
    # —— 这些状态与年龄冲突的关联性弱，且学生也可能填此类
]
...
def build_clean_conditions(questions, include_trap_indices=None):
    ...
    if age_q and job_q:
        ...
        if young_ids and working_ids:
            ...
            rules_applied.append(f"③ 剔除年龄<18岁 且 职业为工作人群（不含自由职业/自雇/无固定工作/兼职）（年龄题: {_strip_html(age_q['title'])[:20]}，职业题: {_strip_html(job_q['title'])[:20]}）")
        else:
...
class SurveyDownloader:
    ...
    def download_file(self, survey_id, data_type, output_dir, begin_ts=None, end_ts=None, survey_name=None):
        """
        下载已导出的文件
        data_type: 0=文本数据, 1=量化数据
        begin_ts/end_ts: 毫秒时间戳，用于文件名中的数据周期
        survey_name: 问卷名称，用于文件名（可选）
        ...
        # 构建文件名：survey_{id}_{问卷名}【文本数据/量化数据】{起始日期}-{结束日期}{ext}
        ...
        if begin_ts and end_ts:
            ...
            date_range = "all"
        filename = f"survey_{survey_id}{name_part}【{type_label}】{date_range}{ext}"
        ...
        if is_data_zip:
            ...
            # 创建 _raw/ 子目录保留原始 ZIP 与分片，避免合并/删除后用户无法回溯
            raw_dir = os.path.join(output_dir, "_raw")
            os.makedirs(raw_dir, exist_ok=True)
            try:
                import shutil as _shutil
                raw_zip_path = os.path.join(raw_dir, os.path.basename(filepath))
                _shutil.copy2(filepath, raw_zip_path)
                _log(f"Preserved raw ZIP: {raw_zip_path}")
            except Exception as e:
                _log(f"WARNING: Failed to preserve raw ZIP: {e}")
            ...
            with zipfile.ZipFile(filepath, 'r') as zf:
                for member in zf.namelist():
                    ...
                    dest_name = f"survey_{survey_id}{name_part}【{type_label}】{date_range}{member_ext}"
                    dest = os.path.join(extract_dir, dest_name)
                    # 如果目标文件已存在，添加序号避免冲突
                    counter = 1
                    while os.path.exists(dest) and dest != src:
                        dest_name = f"survey_{survey_id}{name_part}【{type_label}】{date_range}_{counter}{member_ext}"
                        dest = os.path.join(extract_dir, dest_name)
                        counter += 1
                    if src != dest:
                        os.rename(src, dest)
            ...
            # 合并前先把所有分片复制到 _raw/，因为合并函数内部会删除分片
            try:
                import shutil as _shutil
                for f in extracted_files:
                    _shutil.copy2(f, os.path.join(raw_dir, os.path.basename(f)))
                _log(f"Preserved {len(extracted_files)} raw shard(s) under: {raw_dir}")
            except Exception as e:
                _log(f"WARNING: Failed to preserve raw shards: {e}")

            # 如果有多个分片文件，自动合并
            if len(extracted_files) > 1:
                merged_name = f"survey_{survey_id}{name_part}【{type_label}】{date_range}"
                # 按扩展名分组
    ...
    def download_stat_report(self, survey_id, output_dir, begin_ts=None, end_ts=None, survey_name=None):
        """
        从问卷系统直接下载基础统计报表（各题频率汇总 xlsx）。
        调用 /view/survey_stat/download_questions_stat
        begin_ts/end_ts: 毫秒时间戳
        返回: 下载后的文件绝对路径，失败返回 None
        """
        if not begin_ts or not end_ts:
            _log("download_stat_report: begin_ts/end_ts required, skipping")
            return None

        url = f"{self.base_url}/view/survey_stat/download_questions_stat"
        resp = self.session.get(
            url,
            params={"surveyId": survey_id, "begin": begin_ts, "end": end_ts},
            stream=True,
        )

        if resp.status_code != 200:
            _log(f"Stat report download failed: HTTP {resp.status_code}")
            return None
        ...
        if "json" in ct:
            try:
                err = resp.json()
                _log(f"Stat report API error: {err.get('resultDesc', err)}")
            except Exception:
                pass
            return None
    ...
    def run(self, survey_id=None, survey_name=None, export_type="both",
        ...
        download_stat: 是否同时下载系统统计报表（默认 True）
        """
        ...
        if skip_existing:
            ...
            for dt, dt_name in types_to_export:
                ...
                # 匹配 survey_{id}*【类型】*.csv/.xlsx（兼容新旧命名）
                existing = [
                    f for f in os.listdir(output_dir)
                    if f"survey_{target_id}" in f and f"【{type_label}】" in f
        ...
        # 如果所有原始数据文件都已存在，直接返回（统计报表仍按需下载）
        if not types_need_download:
            _log("All requested files already exist, skipping download entirely")
            if download_stat:
                _log("Downloading system stat report...")
                stat_path = self.download_stat_report(
                    target_id, output_dir, begin_ts, end_ts, survey_name=target_name
                )
                files["stat_data"] = os.path.abspath(stat_path) if stat_path else None
            result = {
                "status": "success",
                "survey_name": target_name,
                "survey_id": target_id,
        ...
        for dt, dt_name in types_need_download:
            ...
            filepath = self.download_file(target_id, dt, output_dir, begin_ts, end_ts, survey_name=target_name)
            if filepath:
        ...
        # 9. 下载系统统计报表（默认开启，可通过 download_stat=False 跳过）
        if download_stat:
            _log("Downloading system stat report...")
            stat_path = self.download_stat_report(
                target_id, output_dir, begin_ts, end_ts, survey_name=target_name
            ...
            files["stat_data"] = os.path.abspath(stat_path) if stat_path else None
            if not stat_path:
                _log("WARNING: Failed to download stat report (non-fatal, raw data still available)")
